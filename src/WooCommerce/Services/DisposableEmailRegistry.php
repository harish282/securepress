 ...
 *  - **WordPress filter `niyiguard.disposable_email_domains`** — applied lazily on
 *    first lookup; lets sites manage the list from a feature plugin or `mu-plugins/`
 *    without modifying NiyiGuard directly.
 *
...
final class DisposableEmailRegistry
{
    ...
    private function ensureFiltersApplied(): void
    {
        ...
        $extra = \call_user_func('apply_filters', 'niyiguard.disposable_email_domains', []);
        if (is_array($extra)) {
            $this->addDomains(array_values(array_filter($extra, 'is_string')));
        }
        $allowed = \call_user_func('apply_filters', 'niyiguard.disposable_email_allowed_domains', []);
        if (is_array($allowed)) {
